/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   defines.h                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: alagache <alagache@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/06/08 15:39:36 by agardina          #+#    #+#             */
/*   Updated: 2020/06/09 12:52:00 by alagache         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef DEFINES_H

# define DEFINES_H

# define TOO_MANY_CHAMP "Too many champions"
# define PLAYER_NUM_ERR "Each champion must have a number between 1 and 4"
# define BAD_FILE_NAME "Each champion file must have a length of minimum 5 and the extension .cor"
# define CODE_SIZE "Code size must be between 1 and 682 bytes"
# define PRINT_USAGE "Usage"

# define SMALL_DIR	2
# define BIG_DIR	4

# define D_FLAG		1
# define DUMP_FLAG	2

#endif
